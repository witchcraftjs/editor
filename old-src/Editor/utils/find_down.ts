...
import { CHILD, Filter, SkipFilter } from "@/components/Editor/types"
...
export function find_down(
	...
	let depth_before = Math.max($from.depth, ($last?.depth ?? 0))
	// let is_self = start === 0

	while ($curr.depth > depth_before || (/* is_self &&  */$curr.start() === from_start)) {
		...
		depth_before = Math.max($last.depth, depth_before)
