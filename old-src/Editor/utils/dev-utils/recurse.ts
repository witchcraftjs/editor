...
import { attrs_to_classes } from "@/components/Editor/utils"
...
export function recurse(node: Node, deep = true, props: ("size" | "is" | "attrs")[] = [], depth = 0): string {
	...
	text.push(`=${node.type.name} ${node.firstChild?.textContent as string}`)
	...
	let bottom_content = Object.entries(obj).map(([key, val]) => key === "children"
		...
		: key === "attrs"
			? `${val ? indent + attrs_to_classes(val as any).trim() : ""}`
