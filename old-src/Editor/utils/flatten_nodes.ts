 ...
 * So it cannot be passed to the plugins array normally, since at that point the editor variable does not yet exist. It must be pushed to the plugins list after the editor view exists. A helper function, `apply_list_fixer` is provided to make this simpler.
 *
 * ```ts
 ...
 * })
 * // manually
 * editor.state.plugins.push(list_fixer({ item_type, container_type, view: editor }))
 * // with helper
 * apply_list_fixer({ item_type, container_type, view: editor })
 * ```
 *
 * Notes:
 ...
 *
 * @param opts.ignore Nodes to not count as empty content nodes.
 */
export function flatten_nodes(content: Node | Fragment, container: NodeType, filter: (node: Node) => boolean): Node {
	let flattened: Node[] = []
